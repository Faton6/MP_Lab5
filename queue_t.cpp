...
class Queue_t
{
    ...
    public:
        ...
        const T &operator[](Iterator_My<T> pos) const { return *pos.ptr; }

        template <typename> 
        class Iterator_My //: public std::iterator<std::input_iterator_tag, T>
        {
            public:
                ...
                Iterator_My() : ptr(nullptr) {}
                Iterator_My(T *ptr) : ptr(ptr) {}

                Iterator_My(const Iterator_My &right) : ptr(right.ptr) {}

                T &operator*() const { return *ptr; }
                T &operator[](size_t pos) const { return ptr[pos]; }
                //T *operator->() const { return ptr->data; }
                bool operator!=(const Iterator_My<T> &right) {return ptr != right.ptr; }
                bool operator==(const Iterator_My<T> &right) {return ptr == right.ptr; }
                bool operator<(const Iterator_My<T> &right)  {return ptr < right.ptr; }
                Iterator_My operator+(const Iterator_My<T> &right) const { return this->ptr + right.ptr; }
                Iterator_My operator+(int right) const { return Iterator_My(this->ptr + right); }
                Iterator_My operator-(const Iterator_My<T> &right) const { return this->ptr - right.ptr; }
                Iterator_My operator-(int right) const { return Iterator_My(this->ptr - right); }
                
                Iterator_My &operator++() {++ptr; return *this; }
                Iterator_My operator++(int) {T *var_ptr = this->ptr; ++ptr; return *var_ptr; }
                Iterator_My &operator--() {--ptr; return *this; }
                Iterator_My operator--(int) {T *var_ptr = this->ptr; --ptr; return *var_ptr; }
        };
...
template <typename T>
Queue_t<T>::Iterator_My<T> Queue_t<T>::begin()
{
    return Iterator_My<T>(arr);
}
template <typename T>
Queue_t<T>::Iterator_My<T> Queue_t<T>::end()
{
    return Iterator_My<T>(arr + this->_size);
}
...
int main()
{
    ...
    mass[4] = 4;
    mass[5] = 5;
    for (Queue_t<int>::Iterator_My<int> it = mass.begin(); it < mass.end(); ++it) std::cout << mass[it] << '\n';
    std::cout << "TEST\n";
}